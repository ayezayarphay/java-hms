...
public class AddStaff implements Initializable {
	...
	public void add() {
		try {
			...
			boolean isAdmin = is_admin.isSelected();
			if (staff_id == 0) {

				Staff staff = new Staff(id_codeT, nameT, "None", img, departmentT, positionT, father_nameT,
						mother_nameT, genderT, dobT, nrcT, degreeT, addressT, start_work_dateT, previous_workT, salaryT,
						isResing, resingDate, isAdmin);
				staffService.insert(staff);
				new Alert(AlertType.INFORMATION, "1 employee added.", ButtonType.OK).showAndWait();
				IDGenerator.increaseStaffID();
			} else {
				Staff staff = new Staff(staff_id, id_codeT, nameT, password.getPassword(), img, departmentT, positionT,
						father_nameT, mother_nameT, genderT, dobT, nrcT, degreeT, addressT, start_work_dateT,
						previous_workT, salaryT, isResing, resingDate, isAdmin);
	...
	public void setData() {
		Staff staff = ObjectCarrior.getStaff();
		password = ObjectCarrior.getStaff();
		if (staff != null) {
			add.setText("Edit");
			staff_id = staffService.getID(staff);
			staff = staffService.findById(staff_id);
			id_code.setText(staff.getId_code());
			name.setText(staff.getName());
			Image img = new Image(new ByteArrayInputStream(staff.getImage()));
			image.setImage(img);
			father_name.setText(staff.getFather_name());
			mother_name.setText(staff.getMother_name());
			department.setValue(staff.getDepartment());
			position.setValue(staff.getPosition());
			if (staff.getSex().equals(Gender.Male)) {
				male.setSelected(true);
			} else {
				female.setSelected(true);
			}
			dob.setValue(staff.getDob());
			nrc.setText(staff.getNrc());
			degree.setText(staff.getDegree());
			address.setText(staff.getAddress());
			start_work_date.setValue(staff.getStart_work_date());
			String get = staff.getPrevious_work();
			if (get.equals("None")) {
				previous_work.setText("");
			} else {
				previous_work.setText(staff.getPrevious_work());
			}
			salary.setText(String.valueOf((int) staff.getSalary()));
			is_resign.setSelected(staff.isResign());
			if (!staff.getResign_date().equals("None")) {
				resignShow();
				resign_date.setValue(LocalDate.parse(staff.getResign_date()));
			}
			is_admin.setSelected(staff.isAdmin());
			ObjectCarrior.setStaff(null);
		}
	}
}
